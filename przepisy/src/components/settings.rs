...
pub fn Settings() -> impl IntoView {
    let set_theme = use_context::<WriteSignal<Theme>>().expect("Brak kontekstu theme");
    let theme = use_context::<ReadSignal<Theme>>().expect("Brak kontekstu theme");
    ...
    let set_settings = use_context::<WriteSignal<Settings>>().expect("Brak ustawien");
    ...
    let toggle_theme = move |_| {
        let new_theme = if theme.get() == Theme::Light { Theme::Dark } else { Theme::Light };
        
        set_theme.set(new_theme.clone());
        set_settings.update(|s| s.theme = new_theme);
    };
    ...
    view! {
        <div class="flex items-center justify-between mb-6 sticky top-4 z-20">
            <button on:click=move |_| { set_view_state.set(CurrentView::Home); set_page_counter_vis.set(true); } class="p-2 rounded-full bg-surface/80 backdrop-blur-md shadow-sm hover:bg-surface text-text-main hover:text-primary transition-all duration-300">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </button>
        </div>
